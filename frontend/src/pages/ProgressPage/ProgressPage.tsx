...
import type { FoodEntry, MealType } from '../../features/progress/types/progress.types'
...
export function ProgressPage() {
  ...
  const [hoveredRing, setHoveredRing] = useState<{ label: string; consumed: number; target: number; color: string } | null>(null)
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        <section className={styles.ringsSection}>
          <div className={styles.ringsContainer}>
            ...
            <ProgressRing
              ...
              size={280}
              strokeWidth={24}
              onHoverStart={() => setHoveredRing({ label: 'Calories', consumed: totals.calories, target: DEFAULT_TARGETS.calories, color: '#f97316' })}
            ...
            <ProgressRing
              ...
              size={200}
              strokeWidth={20}
              onHoverStart={() => setHoveredRing({ label: 'Protein', consumed: totals.protein, target: DEFAULT_TARGETS.protein, color: '#06b6d4' })}
              onHoverEnd={() => setHoveredRing(null)}
            />
            ...
            <ProgressRing
              ...
              size={140}
              strokeWidth={16}
              onHoverStart={() => setHoveredRing({ label: 'Carbs', consumed: totals.carbs, target: DEFAULT_TARGETS.carbs, color: '#8b5cf6' })}
              onHoverEnd={() => setHoveredRing(null)}
            />
          </div>

          {hoveredRing && (
            <div className={styles.fixedTooltip} style={{ backgroundColor: `${hoveredRing.color}40`, border: `1px solid ${hoveredRing.color}` }}>
              <div className={styles.tooltipLabel}>{hoveredRing.label}</div>
              <div className={styles.tooltipValue}>
                {hoveredRing.consumed.toLocaleString()} / {hoveredRing.target.toLocaleString()}
              </div>
            </div>
          )}
        </section>
        ...
        <section className={styles.logSection}>
          ...
          <button className={styles.addButton} onClick={openAddModal} aria-label="Add food entry">
          ...
          {entries.length === 0 && (
            <div className={styles.emptyState}>
              ...
              <p className={styles.emptyText}>No food entries yet. Start tracking your meals!</p>
            </div>
          )}
        </section>
      </div>
